...
int usbhidOpenDeviceByPath ( usbDevice_t **device, const char * path)
{
    if (path == NULL || device==NULL) return -1;

    *device = (usbDevice_t *)CreateFile(path, GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    if ( *device != (usbDevice_t *)INVALID_HANDLE_VALUE)
    {
        return 0;
    }
...
int usbhidEnumDevice(int vendor, char *vendorName, int product, char *productName, proc_dev_filter_t callback)
{
    ...
    for(i=0;;i++){
        ...
        */
        ...
        {
            hid_dev_desc_t desc;
            desc.dev_path =  deviceDetails->DevicePath;
            desc.usb_desc = handle;
            callback(&desc);
        }
...
int usbhidEnumDevice(int vendorID, char *vendorName, int productID, char *productName, proc_dev_filter_t callback)
{
    // not impl yet
    return 0;
}
